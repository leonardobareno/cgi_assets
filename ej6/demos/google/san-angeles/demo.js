...
//Son utilizados en la función ssFunc(t, p):
//m coeficiente para ángulo
//a divisor para x
//b divisor para y
//n1 flotante empleado para radicación
//n2 potencia para x
//n3 potencia para y

//0 a 5:  parámetros para theta = t
//6 a 11: parámetros para phi   = p

//res1: número de polígonos en horizontal (360 grados / res1)
//res2: número de polígonos en vertical   (360 grados / res2)
//scale: factor de escala (se aplicará por igual a los 3 ejes)
var shapeParams = [
	...
	//if cam.dist != 0, rotar alrededor de punto fijo y mantener la "mirada" de la cámara en ese punto
	//0 1 2    3       4
	//c     radianesXY eZ_mag
	/*
	...
	*/
	...
	//if cam.dist == 0, trasladar la cámara, rotarla sobre su centro en plano XY (ie, eje Z)
	//0 1 2    3      4
	//e     radianesXY cZ_mag
	/*
        eX = lerp[0];
	...
	*/
	...
	//cam.len se utiliza como coeficiente: CAMTRACK_LEN * cam.len, con valor 2 la velocidad de movimiento es más lenta
	//src está en coordenadas de mundo, dest está en coordenadas relativas a la cámara

	//************* En JS, 0 equivale a false, notar esto en cam.dist
var camTracks = [
    /*{src:[4500, 2700, 100, 70, -30],       dest:[50, 50, -90, -100, 0],      dist:20, len:1},//cima de obj
    {src:[ -1448, 4294, 25, 363, 0 ],      dest:[ -136, 202, 125, -98, 100], dist:0, len:1},//diag hacia arriba
    {src:[ 1437, 4930, 200, -275, -20 ],   dest:[ 1684, 0, 0, 9, 0],         dist:0, len:1},//trasl piso retrocede
    {src:[ 1800, 3609, 200, 0, 675 ],      dest:[ 0, 0, 0, 300, 0],          dist:0, len:1},//rot y cielo
    {src:[ 923, 996, 50, 2336, -80 ],      dest:[ 0, -20, -50, 0, 170],      dist:0, len:1},//rot x abajo a arriba
    {src:[ -1663, -43, 600, 2170, 0 ],     dest:[ 20, 0, -600, 0, 100],      dist:0, len:1},//trasl eje y
    {src:[ 1049, -1420, 175, 2111, -17 ],  dest:[ 0, 0, 0, -334, 0],         dist:0, len:2},//rot y hacia derecha
    {src:[ 0, 0, 50, 300, 25 ],            dest:[ 0, 0, 0, 300, 0],          dist:70, len:2},//rot around punto
    {src:[ -473, -953, 3500, -353, -350 ], dest:[ 0, 0, -2800, 0, 0],        dist:0, len:2},//eje y hacia piso
    {src:[ 191, 1938, 35, 1139, -17 ],     dest:[ 1205, -2909, 0, 0, 0],     dist:0, len:2},//trasl forward
    {src:[ -1449, -2700, 150, 0, 0 ],      dest:[ 0, 2000, 0, 0, 0],         dist:0, len:2},//pan izq
    {src:[ 5273, 4992, 650, 373, -50 ],    dest:[ -4598, -3072, 0, 0, 0],    dist:0, len:2},//vuelo mirada abajo
    {src:[ 3223, -3282, 1075, -393, -25 ], dest:[ 1649, -1649, 0, 0, 0],     dist:0, len:2}//global, retrocede*/
	//{src:[ 6020, -6000, 1075, -420, -20 ], dest:[ 0, 0, 0, 0, 0],     dist:0, len:2}//queda quieto
	{src:[ -4000, -1400, 3000, 0, -50 ], dest:[ 0, 0, 0, 0, 0],     dist:0, len:2}//queda quieto
	//{src:[ 0, 0, 4000, 0, -1200 ], dest:[ -6000, 6000, 0, 0, 0],     dist:0, len:2}//mirando abajo, vuela diag
	//{src:[ 0, 0, 2000, 0, -1200 ], dest:[ -6000, 6000, 0, 0, 0],     dist:0, len:2}//mirando abajo, vuela diag
	//{src:[ -12000, 0, 0, 0, 0 ], dest:[ 0, 0, 0, 0, 0],     dist:0, len:2}//fuera de conjunto, queda quieto. combinar con figura 1
	//{src:[ -6000, 0, 0, 0, 0 ], dest:[ 0, 0, 0, 0, -100],     dist:0, len:2}//rotar de arriba a abajo
	//{src:[ -6000, 0, 0, 0, 0 ], dest:[ 0, 2000, 0, 0, 0],     dist:0, len:2}//pan izq por fuera del conjunto
	//{src:[ -6000, 0, 0, 0, 500 ], dest:[ 0, 2000, 0, 0, -1500],     dist:0, len:2}//rotar de arriba a abajo
	//{src:[ -6000, 0, 0, 0, 0 ], dest:[ 0, 0, 22000, 0, 0],     dist:0, len:2}//vertical hacia arriba
];//13 movs de cámara

var POLYLEN = 15;
var ANG45   = 0.70710678;
var ALTURA = 10;
//dest está en coordenadas absolutas
var movimientos = [
    //{src:[ 0, 0, 25, 0, 0, 0 ], dest:[ 15, 0, 25, 90, 0, 0], len:1},
    //{src:[ 15, 0, 25, 0, 0, 0 ], dest:[ 15, -15, 25, 0, 0, 0], len:1},
    {src:[ 0,0, ALTURA, 0, 0, 0 ],                                          dest:[ POLYLEN*ANG45, -1.0*POLYLEN*(1.0 - ANG45), ALTURA-5, 0, 90, 45], len:1},
    {src:[ POLYLEN*ANG45,    -1.0*POLYLEN*(1.0 - ANG45), ALTURA-5, 0, 90, 45 ], dest:[ POLYLEN, -1.0*POLYLEN, ALTURA, 0, 0, 90], len:1},
    {src:[ POLYLEN,          -1.0*POLYLEN, ALTURA, 0, 0, 90 ],               dest:[ POLYLEN*ANG45, -1.0*(POLYLEN * (1.0+ANG45)), ALTURA+10, 0, -90, 135], len:1},
    {src:[ POLYLEN*ANG45,    -1.0*(POLYLEN * (1.0+ANG45)), ALTURA+10, 0, -90, 135 ], dest:[ 0, -2.0*POLYLEN, ALTURA, 0, 0, 180], len:1},
    {src:[ 0,                -2.0*POLYLEN, ALTURA, 0, 0, 180 ],               dest:[ -1*POLYLEN*ANG45, -1.0*(POLYLEN * (1.0+ANG45)), ALTURA-5, 0, 90, 225], len:1},
    {src:[ -1*POLYLEN*ANG45, -1.0*(POLYLEN * (1.0+ANG45)), ALTURA-5, 0, 90, 225 ], dest:[ -1*POLYLEN, -1.0*POLYLEN, ALTURA, 0, 0, 270], len:1},
    {src:[ -1*POLYLEN,       -1.0*POLYLEN, ALTURA, 0, 0, 270 ],               dest:[ -1*POLYLEN*ANG45, -1.0*POLYLEN*(1.0 - ANG45), ALTURA+10, 0, -90, 315], len:1},
    {src:[ -1*POLYLEN*ANG45, -1.0*POLYLEN*(1.0 - ANG45), ALTURA+10, 0, -90, 315 ], dest:[ 0, 0, ALTURA, 0, 0, 360], len:1},
...
//LB
var currentObjTrackStartTick = 0;
var nextObjTrackStartTick = 0;
var currentObjTrack = 0;

var litShader = null;//shader para iluminación
var flatShader = null;//shader para el piso
var fadeShader = null;
...
p: array con 6 elementos:
	0 coeficiente para ángulo
	1 divisor para x
	2 divisor para y
	3 flotante empleado para radicación
	4 potencia para x
	5 potencia para y
*/
function ssFunc(t, p) {
...
function createSuperShape(shader, params) {
	...
	//console.log("latitudeBegin="+latitudeBegin);
	//console.log("latitudeEnd="+latitudeEnd);
	//console.log("longitudeCount"+longitudeCount);
	var bLock = 0;
    ...
    var currentVertex = 0;
    for (var longitude = 0; longitude < longitudeCount/2; ++longitude) {
        for (var latitude = latitudeBegin; latitude < latitudeEnd; ++latitude) {
            ...
            var r1 = ssFunc(p1, params.slice(6, 12));//indices 6 a 11, 6 elementos
            var r2 = ssFunc(t2, params);
            var r3 = ssFunc(p2, params.slice(6, 12));
			
			/*if (bLock == 0) {
				bLock = 1;
				console.log("r0(t1)="+r0);
				console.log("r1(p1)="+r1);
				console.log("r2(t2)="+r2);
				console.log("r3(p2)="+r3);
			}*/
            ...
            if (r0 != 0 && r1 != 0 && r2 != 0 && r3 != 0) {
				...
				//superShapeMap almacena coordenadas xyz en el 1er parámetro: pa
				...
				//sustracción de vectores pb-pa y se almacena en v1
                vector3Sub(v1, pb, pa);
                vector3Sub(v2, pd, pa);
...
function drawModels(zScale) {
	...
	//zScale controla si son dibujadas las mitades superiores o las inferiores de los objetos (valores: 1, -1)
    modelview.scale(1, 1, zScale);

	//Al colocar siempre la misma semilla, se generarán siempre los mismos tipos de 
	//shapes en las mismas posiciones
    random.seed(9);
    ...
    for (var x = 0; x <= 0; ++x) {
		...
		modelview.rotate(offs*10, 1., 0., 0.);
        ship.draw();
        modelview.pop();*/
    ...
    var ship = shapes[shapes.length - 1];
	var shipScale100 = translationScale * 500;
	var offs100 = sTick % shipScale100;
	var offs = 0.01 * offs100;
	
	nextObjTrackStartTick = currentObjTrackStartTick +
            movimientos[currentObjTrack].len * OBJTRACK_LEN;
			
	//log("ne " + nextObjTrackStartTick + " " +sTick)
	
    while (nextObjTrackStartTick <= sTick) {
        ++currentObjTrack;
        if (currentObjTrack >= movimientos.length) {
            currentObjTrack = 0;
        }
        currentObjTrackStartTick = nextObjTrackStartTick;
        nextObjTrackStartTick = currentObjTrackStartTick +
                movimientos[currentObjTrack].len * OBJTRACK_LEN;
        // log(currentObjTrack);
    }
	
	var cam = movimientos[currentObjTrack];
    var currentObjTick = sTick - currentObjTrackStartTick;	
	var trackPos = currentObjTick / (OBJTRACK_LEN * cam.len);
	//log("trackPos="+trackPos);
	//log("cam.len="+cam.len);
	//log("sTick="+sTick);
	//log(sTick + " - " +currentObjTrackStartTick + " = " + currentObjTick);
	
	
	
    var lerp = Array(6);
    for (var a = 0; a < 6; ++a) {
        lerp[a] = cam.src[a] + ((cam.dest[a] - cam.src[a]) * trackPos);
		//if (a >= 0 && a <= 2) lerp[a] = 0.01 * lerp[a];
    }	
	
	modelview.push();
	log("lerp[0]="+lerp[0]);
	modelview.translate(lerp[0], lerp[1], lerp[2]);
	//log("offs=" + offs);
	//modelview.translate(offs, -4., 2.);
	modelview.rotate(lerp[3], 1., 0., 0.);
	modelview.rotate(lerp[4], 0., 1., 0.);
	modelview.rotate(lerp[5], 0., 0., 1.);
	ship.draw();
	modelview.pop();
}
...
function camTrack() {
	...
	//log("currentCamTick="+currentCamTick);
    var trackPos = currentCamTick / (CAMTRACK_LEN * cam.len);

    var lerp = Array(5);
	//log("trackPos="+trackPos);
	//trackPos es un flotante en el rango (0.0, 1.0)
    for (var a = 0; a < 5; ++a) {
        lerp[a] = 0.01 * (cam.src[a] + cam.dest[a] * trackPos);
    }
	
	//log("lerp[3]" + lerp[3]);
	//log("cam.dist=" + cam.dist);
...
function drawGroundPlane() {
	//al deshabilitar gl.CULL_FACE y gl.DEPTH_TEST se renderizan todos los pixeles del piso visibles en el frustum
    gl.disable(gl.CULL_FACE);
    gl.disable(gl.DEPTH_TEST);
	
	/*
	Activar fase de blending: combinar salidas de color del fragment shader con las de los buffers de color
	(https://www.khronos.org/opengl/wiki/Blending)
	*/
    gl.enable(gl.BLEND);
	
	/*con esta configuración el color resultante en blending se obtendra: fragmentShaderColor*colorBuffer  */
	...
	/*
	El depth test es activado para condicionar la ejecucion del fragment shader dependiendo de la profundidad.
...
function appInit() {
    ...
    currentObjTrackStartTick = 0;
    nextObjTrackStartTick = 0;	
...
function appRender(tick, width, height) {
    ...
    modelview.push();
    drawModels(-1);//dibujar las mitades inferiores
    modelview.pop();

    drawGroundPlane();//dibujar el piso

    drawModels(1);//dibujar las mitades superiores
